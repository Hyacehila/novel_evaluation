...
from packages.schemas.common.enums import AnalysisMode, ResultStatus, SubmissionSourceType, TaskStatus
...
def test_sqlite_repository_persists_completed_result_across_instances(tmp_path: Path) -> None:
    ...
    assert persisted_task.analysisMode is AnalysisMode.LONG_OPENING_OUTLINE
...
def test_api_restart_keeps_history_and_result_readable(tmp_path: Path, monkeypatch) -> None:
    ...
    with create_client(db_path=db_path, monkeypatch=monkeypatch) as client:
        ...
        assert created["analysisMode"] == "long_opening_outline"
        assert task_payload["status"] == "completed"
        assert task_payload["resultStatus"] == "available"
        assert task_payload["analysisMode"] == "long_opening_outline"
        ...
        assert history_payload["items"][0]["analysisMode"] == "long_opening_outline"
    ...
    with create_client(db_path=db_path, monkeypatch=monkeypatch) as restarted_client:
        ...
        assert restarted_task.json()["data"]["analysisMode"] == "long_opening_outline"
        ...
        assert restarted_history.json()["data"]["items"][0]["analysisMode"] == "long_opening_outline"
...
def test_sqlite_repository_rejects_legacy_result_payload_shape(tmp_path: Path) -> None:
    ...
    legacy_payload = {
        ...
        "result": {
            ...
            "strengths": ["人物动机清晰"],
            "weaknesses": ["开篇冲突偏慢"],
            "platforms": [{"name": "女频平台 A", "percentage": 82, "reason": "题材匹配度较高"}],
            "marketFit": "具备一定市场接受度",
            "editorVerdict": "可继续观察",
            "detailedAnalysis": {
                "plot": "情节推进稳定",
    ...
    with pytest.raises(ValidationError):
        repository.get_result(task.taskId)
...
def test_api_restart_marks_stale_processing_task_failed(tmp_path: Path, monkeypatch) -> None:
    ...
    with create_client(db_path=db_path, monkeypatch=monkeypatch) as client:
        ...
        assert task_response.json()["data"]["analysisMode"] == "long_opening_outline"
...
def test_history_orders_latest_tasks_first_across_restart(tmp_path: Path, monkeypatch) -> None:
    ...
    with create_client(db_path=db_path, monkeypatch=monkeypatch) as client:
        ...
        assert items[0]["analysisMode"] == "long_opening_outline"
        assert items[1]["taskId"] == "task_history_early"
        assert items[1]["analysisMode"] == "long_opening_outline"
