...
import { ensureProviderReady, providerMode, resetRuntimeProviderKey } from "./provider-helpers";
...
const completedFulltext = "全文开场，林照在最后一场审判后回到废弃港城，发现自己三年前提交的航运账本被重新激活。中段里，她逐一追回证人、修复公司账册，并与曾经背叛她的调查员重新结盟。后段通过公开听证、港口封锁和董事会改组收束全部伏笔，最终确认父亲的死亡不是事故，而是跨境走私案的灭口。故事已经给出完整起承转合、关键人物结局和主线谜底。".repeat(3);
...
async function chooseAnalysisMode(page: Page, mode: "long_opening_outline" | "completed_fulltext") {
  const matcher = mode === "long_opening_outline" ? /长篇|开篇.*大纲|大纲.*开篇/ : /全文|完本|已完成/;
...
async function chooseInputMode(page: Page, mode: "direct_input" | "file_upload") {
  const matcher = mode === "direct_input" ? /直接输入/ : /文件上传/;
  const radio = page.getByRole("radio", { name: matcher }).first();
  if (await radio.count()) {
    await radio.check({ force: true });
    return;
...
async function expectCreatePayloadAnalysisMode(page: Page, analysisMode: string) {
  const requestPromise = page.waitForRequest((request) => {
    return request.method() === "POST" && request.url().includes("/api/tasks");
  });
  return requestPromise.then((request) => {
    const payload = request.postDataJSON() as { analysisMode?: string };
    expect(payload.analysisMode).toBe(analysisMode);
  });
}
...
test("首页关键入口按钮可导航", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "选择分析模式，提交小说材料。" })).toBeVisible();
...
test("移动端新建任务页使用紧凑 shell 并快速露出主内容", async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 });
  await page.goto("/tasks/new");

  await expect(page.locator("a.skip-link")).toHaveAttribute("href", "#main-content");
  await expect(page.getByRole("button", { name: /Provider 状态/ })).toBeVisible();
  await expect(page.getByRole("main").getByRole("heading", { name: "选择分析模式，提交小说材料。" })).toBeVisible();

  const mainTop = await page.locator("#main-content").evaluate((element) => element.getBoundingClientRect().top);
  expect(mainTop).toBeLessThan(360);
});

test("长篇开篇加大纲成功流可到达结果页", async ({ page, request }) => {
  ...
  await expect(page.getByRole("textbox", { name: "任务标题" })).toBeVisible();
  await chooseAnalysisMode(page, "long_opening_outline");
  await page.getByRole("textbox", { name: "任务标题" }).fill("Playwright 长篇成功流");
  await page.getByRole("textbox", { name: "正文输入" }).fill(successChapters);
  await page.getByRole("textbox", { name: "大纲输入" }).fill(successOutline);

  const checker = page.getByRole("group", { name: "提交检查器" });
  await expect(page.getByText("正文 + 大纲")).toBeVisible();
  await expect(page.locator("form").getByText("长篇开篇 + 大纲", { exact: true })).toBeVisible();
  await expect(page.getByText("输入已满足模式要求")).toBeVisible();
  await expect(checker).toBeVisible();
  await expect(checker.getByText("正文字符数", { exact: true })).toBeVisible();
  await expect(checker.getByText("大纲字符数", { exact: true })).toBeVisible();
  await expect(checker.getByText("已配置，可进行分析", { exact: true }).first()).toBeVisible();

  const analysisModeRequest = expectCreatePayloadAnalysisMode(page, "long_opening_outline");
  await page.getByRole("button", { name: "创建评测任务" }).click();
  await analysisModeRequest;
...
test("长篇开篇和大纲冲突会进入阻断态且结果页不展示正式结果正文", async ({ page, request }) => {
  test.setTimeout(taskFlowTimeoutMs);
  await resetRuntimeProviderKey(request);
  await ensureProviderReady(page);
  if (providerMode !== "runtime_key") {
  ...
  await chooseAnalysisMode(page, "long_opening_outline");
  await page.getByRole("textbox", { name: "任务标题" }).fill("Playwright 长篇阻断流");
...
test("已完成全文成功流可到达结果页", async ({ page, request }) => {
  test.setTimeout(taskFlowTimeoutMs);
  await resetRuntimeProviderKey(request);
  await ensureProviderReady(page);
  if (providerMode !== "runtime_key") {
  ...
  await chooseAnalysisMode(page, "completed_fulltext");
  await page.getByRole("textbox", { name: "任务标题" }).fill("Playwright 全文成功流");
  await page.getByRole("textbox", { name: "正文输入" }).fill(completedFulltext);
  await expect(page.locator("form").getByText("已完结全文", { exact: true })).toBeVisible();
  await expect(page.getByText("仅正文", { exact: true })).toBeVisible();
  await expect(page.locator("form").getByText("正文字符数", { exact: true })).toBeVisible();
  await expect(page.locator("form").getByText("大纲字符数", { exact: true })).toHaveCount(0);

  const analysisModeRequest = expectCreatePayloadAnalysisMode(page, "completed_fulltext");
  await page.getByRole("button", { name: "创建评测任务" }).click();
  await analysisModeRequest;
  await expect(page).toHaveURL(/\/tasks\/task_/);
  await waitForCompletedTask(page);
...
test("长篇模式缺少大纲时在前端拦截且不会提交请求", async ({ page, request }) => {
  await resetRuntimeProviderKey(request);
  await ensureProviderReady(page);
  if (providerMode !== "runtime_key") {
    await page.goto("/tasks/new");
  ...
  await chooseAnalysisMode(page, "long_opening_outline");
  await page.getByRole("textbox", { name: "任务标题" }).fill("Playwright 输入拦截");
  await page.getByRole("textbox", { name: "正文输入" }).fill(successChapters);
  ...
  const createButton = page.getByRole("button", { name: "创建评测任务" });
...
test("长篇文件上传流能创建任务并保留真实输入语义", async ({ page, request }) => {
  test.setTimeout(taskFlowTimeoutMs);
  await resetRuntimeProviderKey(request);
  await ensureProviderReady(page);
  if (providerMode !== "runtime_key") {
  ...
  await chooseInputMode(page, "file_upload");
  await chooseAnalysisMode(page, "long_opening_outline");
  await page.getByRole("textbox", { name: "任务标题" }).fill("Playwright 长篇上传流");
  ...
  await expect(checker.getByText("chapter.txt", { exact: true })).toBeVisible();
  await expect(checker.getByText("outline.md", { exact: true })).toBeVisible();

  await page.getByRole("button", { name: "创建评测任务" }).click();
  await expect(page).toHaveURL(/\/tasks\/task_/);
  await waitForCompletedTask(page);
  await expect(page.getByText("结果可用", { exact: true })).toBeVisible();
  await expect(page.locator("main").getByText("正文 + 大纲", { exact: true }).first()).toBeVisible();
  await expect(page.locator("main").getByText("长篇开篇 + 大纲", { exact: true }).first()).toBeVisible();
});
